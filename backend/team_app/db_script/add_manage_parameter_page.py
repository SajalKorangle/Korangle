...
def add_manage_parameter_page(apps,schema_editor):
    Task = apps.get_model('team_app', 'Task')
    Module = apps.get_model('team_app','Module')
    EmployeePermission = apps.get_model('employee_app','EmployeePermission')

    student_module = Module.objects.get(path='students')
    manage_parameter = Task(path='manage_parameter',title='Manage Parameter',orderNumber=11,parentModule=student_module)
