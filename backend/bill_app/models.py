...
class Bill(models.Model):
    ...
    name = models.TextField(default='Instalment No. - Session') # Name of the bill to identify like installment, session etc.
    ...
    smsDate = models.DateField(null=True, blank=True)
    smsIntervalDays = models.IntegerField(null=True, blank=True)

    # If the bill is not paid then after this date,
    # software will show modal automatically to all employees.
    modalDate = models.DateField(null=True, blank=True)
    ...
    parentOrder = models.ForeignKey(Order, null=True, related_name='orderList', on_delete=models.SET_NULL, blank=True)
    ...
    if not instance._state.adding and instance.status == 'Completed':
        ...
        if preSavedOrder.status == 'Pending':  # if status changed from 'Pending' to 'Completed'
            try:
                ...
                bill = Bill.objects.get(id=billOrder.parentBill)
