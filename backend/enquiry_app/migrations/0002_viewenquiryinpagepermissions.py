# Generated by Django 3.2.5 on 2023-04-25 02:56

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    dependencies = [
        ('employee_app', '0003_employee_isnonsalariedemployee'),
        ('enquiry_app', '0001_initial'),
    ]

    operations = [
        migrations.CreateModel(
            name='ViewEnquiryInPagePermissions',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('userType', models.CharField(choices=[('Admin', 'Admin'), ('Surveyor', 'Surveyor')], default='Admin', max_length=10)),
                ('parentEmployeePermission', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='employee_app.employeepermission', unique=True, verbose_name='parentEmployeePermission')),
            ],
            options={
                'db_table': 'view_enquiry_in_page_permissions',
            },
        ),
    ]
