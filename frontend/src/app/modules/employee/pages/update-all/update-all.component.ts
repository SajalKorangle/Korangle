...
export class UpdateAllComponent implements OnInit {
  ...
  getParameterValue = (employee, parameter) => {
    try {
      return this.employeeParameterValueList.find(
        (x) =>
          x.parentEmployee === employee.id &&
          x.parentEmployeeParameter === parameter.id
      ).value;
    } catch {
  ...
  getDocumentName(employee, parameter) {
    let item = this.employeeParameterValueList.find(
      (x) =>
        x.parentEmployee === employee.id &&
        x.parentEmployeeParameter === parameter.id
    );
    if (item) {
      if (item.document_name) {
        ...
        return document_name.substring(
          document_name.indexOf("_") + 1,
  ...
  getDocumentIcon(employee, parameter) {
    try {
      let value = this.employeeParameterValueList.find(
        (x) =>
          x.parentEmployee === employee.id &&
          x.parentEmployeeParameter === parameter.id
      ).document_value;
  ...
  checkOnlyUppercaseFields(key: any) {
    return (
      key == "panNumber" || key == "passportNumber" || key == "bankIfscCode"
    );
  }
  ...
  handleOnKeyDown(inputType: any, event: any, backendKey: any) {
    ...
    if (
      inputType == "text" &&
      (backendKey == "panNumber" || backendKey == "passportNumber") &&
      keyPressed == 32
    ) {
      //space
      event.preventDefault(); // if space is pressed prevent Default
    } else if (inputType == "date" && keyPressed != 8 && keyPressed != 46) {
      //check if it is date and not delete
