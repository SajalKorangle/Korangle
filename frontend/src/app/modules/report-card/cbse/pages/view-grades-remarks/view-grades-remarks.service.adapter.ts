    ...
    getStudentFieldRemarkDetails(): void {
        ...
        if (this.vm.showRemark) {
            const student_remark_data = {
                parentSession: this.vm.user.activeSchool.currentSessionDbId,
                parentStudent__in: this.vm.studentSectionList
                    .filter((studentSection) => {
                        return (
                            studentSection.parentClass == this.vm.selectedClassSection.class.id &&
        ...
        this.vm.extraFieldList.forEach((item, index) => {
            ...
            promise_arr.push(this.vm.reportCardCbseService.getObjectList(this.vm.reportCardCbseService.student_extra_field, extra_data));
        });
        this.vm.isLoading = true;

        Promise.all(promise_arr).then(
            (value) => {
                this.vm.currentEmployeesList = this.vm.employeeList.filter(
                    (employee) => value[0].find((obj) => obj.parentEmployee == employee.id) != undefined
                );
                value = value.slice(1);
                if (this.vm.showRemark) {
