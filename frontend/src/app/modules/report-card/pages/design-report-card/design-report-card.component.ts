...
import {
    ...
    UserHandleStructure,
    ALPHABET_LIST,
    marksVariableStructure, MARKS_TYPE_LIST, TEST_TYPE_LIST
} from '@modules/report-card/class/constants';
import DefaultReportCard from '@modules/report-card/class/report-card';
...
export class DesignReportCardComponent implements OnInit {
    ...
    marksTypeList = MARKS_TYPE_LIST;
    alphabetList = ALPHABET_LIST;
    ...
    async updatePDF(delay = 2000) {
        ...
        this.debounceFunction(async () => {
            ...
            if (!this.printMultiple) {
                ...
                card = new DefaultReportCard(this.printMultiple, this.currentLayout, singleStudentData);
            } else {
                card = new DefaultReportCard(this.printMultiple, this.currentLayout, this.data);
            }
            await card.generate();
