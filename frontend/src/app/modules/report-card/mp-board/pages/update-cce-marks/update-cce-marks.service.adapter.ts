...
export class UpdateCceMarksServiceAdapter {
    ...
    initializeData(): void {
        ...
        Promise.all([
            ...
            (value) => {
                ...
                this.vm.examinationOldService.getCCEMarksList(request_cce_marks_data, this.vm.user.jwt).then(
                    (valueTwo) => {
    ...
    getStudentIdList(): any {
        let id_list = [];
        this.student_mini_profile_list
            .filter((student) => {
    ...
    getDataToAdd(): any {
        let data = [];
        this.vm.selectedClass.selectedSection.studentList
            .filter((student) => {
