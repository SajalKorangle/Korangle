...
export class GenerateFinalReportServiceAdapter {
    ...
    getStudentFinalReport(): void {
        ...
        this.vm.subjectService.getClassSubjectList(request_class_subject_data, this.vm.user.jwt).then(valueOne => {
            ...
            let request_student_remarks_data = {
                'parentStudent__in':this.vm.filteredStudentList.filter(student => {return student.selected}).map(a => a.dbId).join(),
    ...
    getExaminationAttendanceStartEndDate(key: any): any {
        ...
        if (this.vm.reportCardMapping[key1] != null && this.vm.reportCardMapping[key2] != null) {
            data = {
                'studentIdList': this.vm.filteredStudentList.filter(student => {return student.selected;}).map(a => a.dbId).join(),
    ...
    extractStudentRemark(studentDbId):string {
        let remark = this.studentRemarksList.find(item => {return item.parentStudent == studentDbId });
