...
export class GradeStudentFieldsServiceAdapter {
    ...
    populateClassSectionList(studentSectionList: any): any {
        ...
        this.classList.forEach((classs) => {
            this.sectionList.forEach((section) => {
                if (
                    this.isClassSectionInPermissionList(classs, section) &&
    ...
    isClassSectionInPermissionList(classs: any, section: any): boolean {
        ...
        this.permissionList.every((item) => {
            if (item.parentClass === classs.id && item.parentDivision === section.id) {
                result = true;
                return false;
            }
            return true;
    ...
    getStudentFieldDetails(): void {
        ...
        this.vm.examinationService.getObjectList(this.vm.examinationService.student_extra_sub_field, request_student_field_data).then(
            (value2) => {
                console.log(value2);
    ...
    getStudentSubField(student: any, subField: any, student_sub_field_list: any): any {
        ...
        student_sub_field_list.every((item) => {
            if (item.parentStudent === student.id && item.parentExtraSubField === subField.id) {
                result = item;
                return false;
            }
            return true;
    ...
    updateStudentField(studentSubField, current_value): any {
        ...
        if (studentSubField.marksObtained > 2.0 || studentSubField.marksObtained < 0.0) {
        ...
        if (studentSubField.id == 0) {
            ...
            service_list.push(
                this.vm.examinationService.createObject(this.vm.examinationService.student_extra_sub_field, request_studentSubField_data)
            );
        } else {
        ...
        Promise.all(service_list).then(
            (value) => {
                ...
                document
                    .getElementById(studentSubField.parentStudent + '_' + studentSubField.parentExtraSubField)
    ...
    updateAllRemainingStudentFields(): void {
        if (this.vm.minimumMarks < 0.0 || this.vm.minimumMarks > 2.0) {
            this.vm.minimumMarks == 0.0;
        }
        if (this.vm.maximumMarks < 0.0 || this.vm.maximumMarks > 2.0) {
            this.vm.minimumMarks == 2.0;
        }
        if (this.vm.maximumMarks < this.vm.minimumMarks) {
            this.vm.minimumMarks == 0.0;
            this.vm.maximumMarks == 0.0;
        }

        let request_student_subfield_data = [];

        this.vm.getFilteredStudentList().forEach((student) => {
            student.subFieldList
                .filter((subField) => {
                ...
                .forEach((studentSubField) => {
                    ...
                    document
                        .getElementById(studentSubField.parentStudent + '_' + studentSubField.parentExtraSubField)
                        .classList.add('updatingField');
                });
        });

        this.vm.examinationService.createObjectList(this.vm.examinationService.student_extra_sub_field, request_student_subfield_data).then(
            (value) => {
                this.vm.getFilteredStudentList().forEach((student) => {
                    student.subFieldList
                        .filter((subField) => {
                        ...
                        .forEach((studentSubField) => {
                            let studentSubFieldNew = value.find((item) => {
                                return (
                                    item.parentStudent == studentSubField.parentStudent &&
                            ...
                            if (studentSubFieldNew.marksObtained == 0.0) {
