...
export class RecordPaymentServiceAdapter {
    ...
    generateEmployeePayment(): void {
        if (this.vm.employeeDetails.amount === null ||
            this.vm.employeeDetails.amount === '' ||
            this.vm.employeeDetails.amount === 0) {
            alert('Amount can not be null or zero.');
            return;
        }
        console.log(this.vm.employeeDetails);
        this.vm.isLoading = true;
        this.vm.salaryService.createEmployeePayment(this.vm.employeeDetails, this.vm.user.jwt).then(value => {
            console.log(value);
            alert(value.message);
            this.vm.employeeDetails = {
                parentEmployee: this.vm.selectedEmployee.id,
    ...
    deleteEmployeePayment(record: any): void {
        ...
        this.vm.isLoading = true;
        this.vm.salaryService.deleteEmployeePayment(data, this.vm.user.jwt).then(value => {
            alert(value);
