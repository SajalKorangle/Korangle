...
export class LayoutSharingDialogComponent implements OnInit {
    ...
    constructor(@Inject(MAT_DIALOG_DATA) public data: { [key: string]: any }) {
        this.vm = data.vm;
        if (this.vm.currentLayout.id) this.layoutSharingList = this.vm.layoutSharingData[this.vm.currentLayout.id];
    }
    ...
    getSchoolList(): void {
        ...
        this.vm.serviceAdapter.getSchoolList(requestData).then((response) => {
            this.schoolList = response.filter(
                (school) =>
                    school.id != this.vm.user.activeSchool.dbId && this.layoutSharingList.every((ls) => ls.parentSchool != school.id)
            );
