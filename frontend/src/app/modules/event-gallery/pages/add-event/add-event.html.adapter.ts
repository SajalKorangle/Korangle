...
export class AddEventHtmlAdapter {
        ...
        this.vm.notifySelectionList.forEach((notifyTo) => {
            notifyTo.selected =
                !!this.vm.eventNotifyList.find(
                    (eventNotify) => eventNotify.parentEvent === event.id && eventNotify.parentClass === notifyTo.id
                ) ||
                (notifyTo.name == 'Employees' && event.notifyEmployees);
        });
    ...
    areInputsValid(): boolean {
        if (this.vm.editing && this.vm.editingEvent.title && this.vm.editingEvent.description) {
            return (
                this.vm.editingEvent.title !== '' &&
                this.vm.editingEvent.title.trim() !== '' &&
                this.vm.editingEvent.description !== '' &&
                this.vm.editingEvent.description.trim() !== ''
            );
        } else {
            return (
                this.vm.newEvent.title &&
                this.vm.newEvent.description &&
                this.vm.newEvent.title !== '' &&
                this.vm.newEvent.title.trim() !== '' &&
                this.vm.newEvent.description !== '' &&
                this.vm.newEvent.description.trim() !== ''
            );
        }
    }
    ...
    checkForChanges() {
        return (
            JSON.stringify(this.vm.eventList.find((event) => event.id == this.vm.editingEvent.id)) !=
                JSON.stringify(this.vm.editingEvent) ||
            JSON.stringify(this.vm.notifySelectionList) != JSON.stringify(this.vm.editingNotificationList)
        );
    }
}
