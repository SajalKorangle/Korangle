...
export class PromoteStudentServiceAdapter {
        ...
        Promise.all([
            ...
            (value) => {
                ...
                this.vm.studentSectionListOne = value[2].filter(
                    (ss) => value[10].find((tc) => tc.parentStudentSection == ss.id) == undefined
                );
    ...
    promoteStudents(): void {
        ...
        this.vm.classSubjectList
            .filter((classSubject) => {
                return (
                    classSubject.parentClass == this.vm.toSelectedClass.id && classSubject.parentDivision == this.vm.toSelectedSection.id
                );
            })
        ...
        this.vm.testSecondList
            .filter((testSecond) => {
                return testSecond.parentClass == this.vm.toSelectedClass.id && testSecond.parentDivision == this.vm.toSelectedSection.id;
            })
            .forEach((testSecond) => {
                this.vm.newPromotedList.forEach((studentSection) => {
