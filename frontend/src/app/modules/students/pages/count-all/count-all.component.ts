        ...
        for (const filter in filtersData) {
            ...
            if (filter == "selectedClass") {  /* Class Check */
                ...
                if (student.dateOfBirth) {
                    let current_date = new Date(filtersData[filter][0]).getDate();
                    let current_month = new Date(filtersData[filter][0]).getMonth() + 1;
                    let current_year = new Date(filtersData[filter][0]).getFullYear();
