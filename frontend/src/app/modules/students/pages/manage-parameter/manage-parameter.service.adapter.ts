...
export class ManageParameterServiceAdapter {
    ...
    saveParameter(): void {
        ...
        if (this.vm.currentParameter.id) {
            ...
            if (this.vm.currentParameter.parameterType == this.vm.customParameterTypeList[1]) {
                this.vm.oldFilterValueList
                    .filter((item) => !this.vm.currentParameter.filterValues.includes(item))
                    .forEach((item) => {
                        service_list.push(
                            this.vm.studentService.deleteObjectList(this.vm.studentService.student_parameter_value, {
    ...
    deleteParameter = () => {
        ...
        this.vm.isLoading = true;
        this.vm.studentService.deleteObject(this.vm.studentService.student_parameter, parameter).then(
            (val) => {
