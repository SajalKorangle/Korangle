...
const CATEGORY_LIST = ['SC', 'ST', 'OBC', 'Gen.'];

const RTE_LIST = ['YES', 'NO'];

const BLOOD_GROUP_LIST = ['A -', 'A +', 'B -', 'B +', 'AB -', 'AB +', 'O -', 'O +'];

const RELIGION_LIST = ['Hinduism', 'Islam', 'Jainism', 'Christianity'];
...
export class UpdateAllComponent implements OnInit {
    ...
    getParameterValue = (student, parameter) => {
        try {
            return this.studentParameterValueList.find((x) => x.parentStudent === student.dbId && x.parentStudentParameter === parameter.id)
                .value;
        } catch {
