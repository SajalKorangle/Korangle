...
export class UpdateProfileServiceAdapter {
    ...
    updateProfile(): void {
        ...
        this.vm.studentParameterList.forEach(parameter => {
            if(this.vm.checkCustomFieldChanged(this.vm.currentStudent, parameter)){
                let temp_obj = this.vm.currentStudentParameterValueList.find(x => x.parentStudent===this.vm.currentStudent.id && x.parentStudentParameter===parameter.id)
                if (temp_obj.id){
    ...
    getStudentProfile(studentId : any): void {
        ...
        this.vm.studentService.getObject(this.vm.studentService.student, data).then(value=>{
            this.vm.currentStudent = this.vm.commonFunctions.copyObject(value);
            Object.keys(value).forEach(key => {
                this.vm.selectedStudent[key] = value[key];
            });
            this.vm.currentStudentSection = this.vm.commonFunctions.copyObject(this.vm.selectedStudentSection);
            // Copying the student aprameter values for reference
            this.vm.currentStudentParameterValueList = []
