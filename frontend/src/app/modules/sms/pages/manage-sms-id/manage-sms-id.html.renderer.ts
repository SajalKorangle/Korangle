...
export class ManageSmsIdHtmlRenderer {
    ...
    invalidSMSIdDetails() {
        if (this.vm.backendData.existingSMSIdDetails && !this.smsIdAlreadyExist()) {
            if (this.vm.userInput.newSMSId.entityRegistrationId &&
                this.vm.userInput.newSMSId.entityRegistrationId != this.vm.backendData.existingSMSIdDetails.entityRegistrationId) {
                return true;
            }
        }
        return false;
