...
@Component({
    ...
    providers: [TCService, StudentService, ClassService, EmployeeService],
})
export class TCLogbookComponent implements OnInit {
    ...
    populateStudentSectionWithTC(): void {
        ...
        this.studentSestionWithTC.sort((ss1, ss2) => {
            const class1 = this.classList.find((c) => c.id == ss1.parentClass);
            const class2 = this.classList.find((c) => c.id == ss2.parentClass);
            if (class1.orderNumber == class2.orderNumber) {
                const division1 = this.divisionList.find((d) => d.id == ss1.parentDivision);
                const division2 = this.divisionList.find((d) => d.id == ss2.parentDivision);
                return division1.orderNumber - division2.orderNumber;
            }
            return class1.orderNumber - class2.orderNumber;
        });
    }
    ...
    getFilteredStudentSectionList(): Array<CustomSSInterface> {
        return this.studentSestionWithTC.filter(
            (ss) => this.classSectionList.find((cs) => cs.class.id == ss.parentClass && cs.section.id == ss.parentDivision).selected
        );
    }
