...
export class UpdateTransactionDialogComponent implements OnInit {
    ...
    isAmountUnEqual(): boolean {
        ...
        this.transaction.creditAccounts.forEach((account) => {
            if (account.amount != null) totalCreditAmount += account.amount;
        });
        let totalDebitAmount = 0;
        this.transaction.debitAccounts.forEach((account) => {
            if (account.amount != null) totalDebitAmount += account.amount;
        });
...
isAddButtonDisabled(): boolean {
    if (
        ...
        this.isAmountUnEqual() ||
        this.isAccountNotMentioned() ||
        this.isAccountRepeated() ||
        this.isAmountLessThanMinimum()
    ) {
        return true;
    }
    return false;
...
addTransaction() {
    ...
    Promise.all([this.vm.accountsService.partiallyUpdateObject(this.vm.accountsService.transaction, transaction_data)]).then(
        (value1) => {
            ...
            this.transaction.debitAccounts.forEach((account) => {
                let index = this.originalTransaction.debitAccounts
                    .map(function (e) {
                        return e.dbId;
                    })
                    .indexOf(account.dbId);
                if (index == -1) {
                    let tempData = {
            ...
            this.transaction.creditAccounts.forEach((account) => {
                let index = this.originalTransaction.creditAccounts
                    .map(function (e) {
                        return e.dbId;
                    })
                    .indexOf(account.dbId);
            ...
            this.transaction.billImages.forEach((image) => {
                ...
                if (image.id != null) {
                    index = this.originalTransaction.billImages
                        .map(function (e) {
                            return e.id;
                        })
                        .indexOf(image.id);
            ...
            this.transaction.quotationImages.forEach((image) => {
                ...
                if (image.id != null) {
                    index = this.originalTransaction.quotationImages
                        .map(function (e) {
                            return e.id;
                        })
                        .indexOf(image.id);
            ...
            service.push(
                this.vm.accountsService.partiallyUpdateObjectList(
            ...
            if (toDeleteAccountList.length > 0) {
                service.push(
                    this.vm.accountsService.deleteObjectList(this.vm.accountsService.transaction_account_details, delete_data)
                );
            }
            service.push(
                this.vm.accountsService.partiallyUpdateObjectList(this.vm.accountsService.transaction_images, toUpdateImageList)
            );
