...
@Component({
    ...
    providers: [AccountsService, EmployeeService, SchoolService],
})
        ...
        if (!this.showSelectedOnly) {
            ...
            list.forEach((transaction) => {
                ...
                this.filterAccountsList.forEach((account) => {
                    if (account.selected) {
                        if (
                            transaction.debitAccounts.find((acccount) => acccount.accountDbId == account.accountDbId) == undefined &&
        ...
        list.forEach((trans) => {
            ...
            trans.debitAccounts.forEach((account) => {
                if (account.parentGroup == null && this.filterGroupsList[0].selected) {
                    ...
                    account.parentGroup != null &&
                    this.filterGroupsList.find((group) => group.groupDbId == account.parentGroup).selected
                ) {
                    containGroup = true;
                }
            });
