...
export class EditAccountDialogComponent implements OnInit {
    ...
    editAccount() {
        ...
        Promise.all([
            ...
            const indexOfCustomAccountSession = this.data.vm.accountsList.findIndex(
                (accountSession) => accountSession.id == this.account.id
            );
    ...
    isAccountDeletable() {
        ...
        Promise.all([this.data.vm.accountsService.getObjectList(this.data.vm.accountsService.transaction_account_details, data)]).then(
            (val) => {
                console.log(val);
                if (val[0].length == 0) {
    ...
    deleteAccount() {
        ...
        console.log(this.account);
        Promise.all([this.data.vm.accountsService.deleteObject(this.data.vm.accountsService.account_session, this.account)]).then((val) => {
