...
export class AddGroupDialogComponent implements OnInit {
  ...
  addGroup(): any {
    ...
    addGroup(): any {
      ...
      Promise.all([this.data.vm.accountsService.createObject(this.data.vm.accountsService.accounts, group_data)]).then((value) => {
        this.data.vm.backendData.accountsList.push(value[0]);
        console.log(value);
        let group_session_data = {
          parentAccount: value[0].id,
        ...
        Promise.all([this.data.vm.accountsService.createObject(this.data.vm.accountsService.account_session, group_session_data)]).then(
          (data) => {
            data[0]['type'] = 'GROUP';
            const customAccount = { ...data[0], type: 'GROUP', title: value[0].title };
            this.data.vm.groupsList.push(customAccount);
