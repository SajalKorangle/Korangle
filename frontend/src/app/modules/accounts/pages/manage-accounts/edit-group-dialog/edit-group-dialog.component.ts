...
deleteGroup(){
    ...
    findGroupHierarchy(root) {
        // dfs on hierarchy structure
        if (root.type == 'ACCOUNT') return null;
        if (root.id == this.group.id) return root;
        let group = null;
        root.childs.every((g) => {
    ...
    editGroup() {
        ...
        allChilds.forEach((accountSessionId) => {
            ...
            if (!accountSesion) accountSesion = this.data.vm.accountsList.find((a) => a.id == accountSessionId);
        ...
        Promise.all([
            ...
            this.data.vm.backendData.accountsList[backendAccountIndex] = {
                ...this.data.vm.backendData.accountsList[backendAccountIndex],
                ...val[1],
            };

            const indexOfCustomGroupSession = this.data.vm.groupsList.findIndex((groupSession) => groupSession.id == val[0].id);
            this.data.vm.groupsList[indexOfCustomGroupSession] = {
                ...this.data.vm.groupsList[indexOfCustomGroupSession],
                ...val[0],
                title: val[1].title,
            };
    ...
    deleteGroup() {
        ...
        this.isLoading = true;
        Promise.all([this.data.vm.accountsService.deleteObject(this.data.vm.accountsService.account_session, this.group)]).then((val) => {
            ...
            Promise.all([
                ...
                if (data[0].length == 0) {
                    ...
                    Promise.all([this.data.vm.accountsService.deleteObject(this.data.vm.accountsService.accounts, account_data)]).then(
                        (value) => {
                            this.data.vm.backendData.accountsList = this.data.vm.backendData.accountsList.filter((acc) => {
                                return acc.id != value[0];
                            });
