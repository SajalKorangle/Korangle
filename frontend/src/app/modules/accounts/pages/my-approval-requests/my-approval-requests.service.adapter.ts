...
export class MyApprovalRequestsServiceAdapter {
    ...
    initialiseData() {
        ...
        Promise.all([
            ...
            Promise.all([this.vm.accountsService.getObjectList(this.vm.accountsService.approval, approval_request_data)]).then((val) => {
        ...
        approvalList.forEach((approval) => {
            ...
            approvalImages.forEach((image) => {
                if (image.parentApproval == approval.id) {
                    if (approval.parentTransaction == null && approval.requestStatus == 'APPROVED') {
    ...
    requestApprovals(): any {
        ...
        Promise.all([this.vm.accountsService.getObjectList(this.vm.accountsService.approval, data)]).then((val) => {
            console.log(val);
            if (val[0].length > 0) {
            ...
            this.vm.newApprovalList.forEach((approval) => {
                ...
                nextApprovalID = nextApprovalID + 1;
                toCreateList.push(tempData);
            });
            Promise.all([this.vm.accountsService.createObjectList(this.vm.accountsService.approval, toCreateList)]).then((value) => {
                console.log(value);
                ...
                services.push(
                    this.vm.accountsService.createObjectList(this.vm.accountsService.approval_account_details, toCreateAccountList)
                );
