...
export class TransferBalanceServiceAdapter {
    ...
    populateCurrentSessionAccountSessionList(currentAccountsSessionList: Array<AccountSession>) {
        ...
        for (let i = 0; i < currentAccountsSessionList.length; i++) {
            ...
            const nextSessionAccount = this.vm.nextSessionAccountSessionList.find(
                (acc) => acc.parentAccount == accountSession.parentAccount
            );
            if (nextSessionAccount != undefined && acc.accountType == 'ACCOUNT') {
                ...
                } else if (accountSession.balance == nextSessionAccount.openingBalance) {
    ...
    initialiseDisplayData(toPopulateAccountsSessionList, list) {
        const nonIndividualAccount = JSON.parse(
            JSON.stringify(
                toPopulateAccountsSessionList.filter((accountSession) => accountSession.parentGroup && accountSession.type == 'ACCOUNT')
            )
        );
        ...
        const groupStructureList = JSON.parse(
            JSON.stringify(toPopulateAccountsSessionList.filter((accountSession) => accountSession.type == 'GROUP'))
        ).map((group) => {
