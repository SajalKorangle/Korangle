...
export class TransferBalanceServiceAdapter{
    ...
    initialiseDisplayData(toPopulateAccountsSessionList, list){
        const nonIndividualAccount = JSON.parse(JSON.stringify(toPopulateAccountsSessionList.filter(accountSession=>accountSession.parentGroup && accountSession.type=='ACCOUNT')));
        const individualAccountList = JSON.parse(JSON.stringify(toPopulateAccountsSessionList.filter(accountSession => !accountSession.parentGroup && accountSession.type=='ACCOUNT')));
        const groupStructureList = JSON.parse(JSON.stringify(toPopulateAccountsSessionList.filter(accountSession=> accountSession.type=='GROUP'))).map(group => {
        ...
        for(let i=0; i<groupStructureList.length; i++){
            if (groupStructureList[i].parentGroup) {
                if (groupStructureList.find(group => group.id == groupStructureList[i].parentGroup) == undefined)
                    console.log('groupSructre[i] = ', groupStructureList[i]);
