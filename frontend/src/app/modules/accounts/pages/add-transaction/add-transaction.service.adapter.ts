...
export class AddTransactionServiceAdapter {
                ...
                if (value.length == 1) {
                    ...
                    Promise.all([
                        ...
                        Promise.all([
                            ...
                            this.vm.accountsService.getObjectList(
                                this.vm.accountsService.approval_account_details,
                                approval_account_details
                            ),
    ...
    addTransactions(): any {
        ...
        Promise.all([this.vm.accountsService.createObjectList(this.vm.accountsService.transaction, toCreateTransactionList)]).then(
            (value) => {
                ...
                Promise.all(serviceList).then((data) => {
                    this.vm.backendData.approvalList = this.vm.backendData.approvalList.filter((approval) => {
                        return (
                            this.vm.transactionList.find((transaction) => transaction.approval && transaction.approval.id == approval.id) ==
                            undefined
                        );
                    });
