...
describe('ManageParameterComponent', () => {
    ...
    it('Component -> addFilter() already exists', fakeAsync( () =>  {
        component.currentParameter = {
            'id': 1,
            'parentSchool': 1,
            'name': 'Test Parameter',
            'parameterType': component.customParameterTypeList[0],
            'filterValues': [ 'Testing' ]
        };
        window.alert = () => { return true; };
        component.addFilter('Testing');
        expect(component.currentParameter.filterValues.length).toBe( 1);
    }));
